...
| 标度 | 含义 | 详表见 `assets/references/saaty-scale.md` |
| --- | --- | --- |
| 1 | 同等重要 |  |
| 3 | 稍重要 |  |
| 5 | 明显重要 |  |
...
> 门禁兜底：调 `assets/scripts/ahp_gate.py` 用代码算主特征向量与 CR，不许手算。跑不过 `CR<0.1` 直接 HOLD。

每张矩阵（用代码算，步骤展示）：

1. 主特征值 λ_max 与特征向量 → 归一化权重 w（`ahp_gate.py` 主特征向量法）。
2. CI = (λ_max − n) / (n − 1)。
3. CR = CI / RI，RI 用标准表：

| n | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 详表见 `assets/references/ri-table.md` |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
